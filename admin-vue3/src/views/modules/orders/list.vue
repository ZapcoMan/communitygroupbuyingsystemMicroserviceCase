  ...
  <div class="list-container">
        ...
        <el-table-column label="操作" width="280" fixed="right">
          <template #default="scope">
            ...
            <el-button 
              ...
              type="text" 
              size="small" 
              @click="handleUpdateStatus(scope.row.id, '已支付')"
            >
              确认支付
            </el-button>
            <el-button 
    ...
    <el-dialog v-model="detailVisible" title="订单详情" width="600px">
      <el-descriptions :column="2" border v-if="orderDetail">
        ...
        <el-descriptions-item label="商品名称">{{ orderDetail.goodname }}</el-descriptions-item>
        <el-descriptions-item label="单价">￥{{ orderDetail.price }}</el-descriptions-item>
        <el-descriptions-item label="数量">{{ orderDetail.buynumber }}</el-descriptions-item>
        <el-descriptions-item label="总价">￥{{ orderDetail.total }}</el-descriptions-item>
        <el-descriptions-item label="订单状态">
          <el-tag :type="getStatusType(orderDetail.status)">{{ orderDetail.status }}</el-tag>
        </el-descriptions-item>
        <el-descriptions-item label="商品类型">{{ orderDetail.tablename }}</el-descriptions-item>
        <el-descriptions-item label="下单时间">{{ orderDetail.addtime }}</el-descriptions-item>
        <el-descriptions-item label="收货地址" :span="2">{{ orderDetail.address || '-' }}</el-descriptions-item>
        <el-descriptions-item label="联系电话">{{ orderDetail.tel || '-' }}</el-descriptions-item>
      </el-descriptions>
    </el-dialog>
  </div>
</template>
...
const handleUpdateStatus = async (id, status) => {
  try {
    ...
    const res = await request.put('/order/orders', { id, status })
    if (res.code === 0) {
...
const handleDelete = async (id) => {
  try {
    ...
    const res = await request.delete('/order/orders/' + id)
    if (res.code === 0) {
...
const handleBatchDelete = async () => {
  ...
  try {
    ...
    const res = await request.delete('/order/orders/batch', { data: ids })
    if (res.code === 0) {
